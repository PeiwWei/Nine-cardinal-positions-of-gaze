...
class ISA_Block_long(nn.Module):
    def __init__(
            self,
            d_model,
            nhead,
    ...
    def forward(self, q, k, v, pos):
        ...
        out = self.self_attn_long(q + pos, k + pos, v)[0]
...
class ISA_Block_short(nn.Module):
    def __init__(
            self,
            d_model,
            nhead,
    ...
    def forward(self, q, k, v, pos):
        ...
        bn_h, bn_w = self.block_num
        pos = pos.repeat(B, 1, 1)
        # new_num_heads = num_heads * (h_b * w_b) / (bn_h * bn_w)
...
class TransformerEncoderLayer(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, activation="relu", block_size=[12, 4], block_num = [3, 3], mlp_ratio=1.):
        super().__init__()
        self.self_attn_long = ISA_Block_long(d_model, nhead, dropout=dropout, block_size=block_size, block_num = block_num)
        self.self_attn_short = ISA_Block_short(d_model, nhead, dropout=dropout, block_size=block_size, block_num=block_num)
        ...
        self.linear3 = nn.Linear(d_model, self.mlp_hidden_dim)
        self.dropout_2 = nn.Dropout(dropout)
        self.linear4 = nn.Linear(self.mlp_hidden_dim, d_model)
    ...
    def forward(self, src, pos):
        B, N, C  = src.shape
        src2 = self.self_attn_long(src, src, src, pos)
        src = src.view(B, N, C).permute(0, 1, 2).contiguous()
        src2 = src2.view(B, N, C).permute(0, 1, 2).contiguous()

        src = src + self.dropout1(src2)
        src = self.norm1(src)
        src2 = self.linear2(self.dropout_1(self.activation(self.linear1(src))))
        ...
        src2 = self.self_attn_short(src, src, src, pos)
        src = src.view(B, N, C).permute(0, 1, 2).contiguous()
        src2 = src2.view(B, N, C).permute(0, 1, 2).contiguous()

        src = src + self.dropout3(src2)
        src = self.norm3(src)
        src2 = self.linear4(self.dropout_2(self.activation(self.linear3(src))))
